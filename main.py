...
def initialize_repository(tracking_repo_path):
    try:
        if os.path.exists(os.path.join(tracking_repo_path, '.git')):
            logger.info(f"Tracking repository already exists at {tracking_repo_path}")
            return Repo(tracking_repo_path)
        else:
            ...
            with open(readme_path, 'w') as f:
                ...
                f.write('This repository tracks changes from another repository and contains summaries generated by AWS Bedrock.\n')
...
def summarize_changes_using_bedrock(changes):
    ...
    try:
        ...
        response = bedrock_runtime.invoke_model(
            modelId="anthropic.claude-3-5-haiku-20240307-v1:0",  # Claude 3.5 Haiku model ID
            body=json.dumps({
